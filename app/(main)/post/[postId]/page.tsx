...
import { useGetCurrentUser, useGetPostById, usePostComment } from "@/lib/react-query/queries-mutation"
...
const SpecificPost = ({ params } : { params: { postId: string }}) => {
  const { data: post, isLoading: isPostLoading, isFetching } = useGetPostById(params.postId || "")
  const { data: user } = useGetCurrentUser()
  const [isOpen, setIsOpen] = useState(false);
  const [comment, setComment] = useState<string>("");

  const { mutateAsync: postComment, isPending: isPostPending  } = usePostComment()
  ...
  return (
    <section className="pb-16">
      <header className="h-[44px] py-2 w-full">
        <nav className="flex px-4 items-center h-full w-full">
          <button className="" onClick={() => router.back()}>
            <Image src="/assets/back.svg" width={9} height={18} alt="back" />
          </button>
      ...
      <div className="flex gap-x-[10px] items-center justify-between px-2 py-2">
          ...
          <Image src={post?.imageUrl} alt="post image" width={375} height={375} className="object-cover aspect-square" />
        </div>
